    ...
    async (data: { runId?: string; actionIds?: string[]; project?: string }) => {
      if (isNoopProvider(provider)) {
        return {
          success: false,
          disabled: true,
          error: "flow compression requires an enabled LLM provider",
          compressed: 0,
        };
      }
      ...
      try {
        ...
        if (!response.trim()) {
          return {
            success: false,
            error: "compression provider returned an empty response",
            compressed: 0,
          };
        }
        ...
        if (
          ...
          !summary.steps?.trim() &&
          !summary.discoveries?.trim() &&
          !summary.lesson?.trim()
        ) {
          return {
            success: false,
            error: "compression provider returned no usable summary fields",
            compressed: 0,
          };
        }
