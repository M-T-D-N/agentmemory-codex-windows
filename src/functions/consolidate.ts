  ...
  sdk.registerFunction("mem::consolidate", 
    async (data: { project?: string; minObservations?: number }) => {
      ...
      const filtered = data.project
        ? visibleSessions.filter((s) => s.project === data.project)
        : visibleSessions;
      ...
      for (let i = 0; i < filtered.length; i++) {
        for (const obs of obsPerSession[i]) {
          const visible = sanitizeCodexAmbientObservation(obs);
          if (visible?.title && visible.importance >= 5) {
            allObs.push({ ...visible, sid: filtered[i].id });
          }
        }
      }
      ...
      for (const [concept, obsGroup] of sortedGroups) {
        ...
        try {
          ...
          if (existingMatch) {
            ...
            await safeAudit(kv, "evolve", "mem::consolidate", [existingMatch.id], {
            ...
            vectorIndexRemove(existingMatch.id);
            ...
            await safeAudit(kv, "evolve", "mem::consolidate", [evolved.id], {
            ...
            scheduleIndexSave();
            await safeAudit(kv, "remember", "mem::consolidate", [memory.id], {
