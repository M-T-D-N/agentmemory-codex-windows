...
export function registerReflectFunctions(
    ...
    async (data: { maxClusters?: number; project?: string }) => {
      ...
      if (!project || project === "*" || project.length > 512) {
        return {
          success: false,
          disabled: false,
          error: "exact project is required for reflection",
        };
      }
      if (isNoopProvider(provider)) {
        return {
          success: false,
          disabled: true,
          error: "reflection requires an enabled LLM provider",
        };
      }
      ...
      const [allGraphNodes, allGraphEdges, allSemanticMemories, lessons, allCrystals] =
        await Promise.all([
      ...
      const graphNodes = allGraphNodes.filter((node) => node.project === project);
      const graphNodeIds = new Set(graphNodes.map((node) => node.id));
      const graphEdges = allGraphEdges.filter(
        (edge) =>
      ...
      const projectSessionIds = new Set(
        (await kv.list<import("../types.js").Session>(KV.sessions))
          .filter(
            (session) =>
      ...
      const semanticMemories = allSemanticMemories.filter((memory) =>
        memory.sourceSessionIds.some((sessionId) => projectSessionIds.has(sessionId)),
      );
      const activeLessons = lessons.filter(
        (lesson) => !lesson.deleted && lesson.project === project,
      );
      const crystals = allCrystals.filter((crystal) => crystal.project === project);
  ...
  sdk.registerFunction("mem::insight-decay-sweep", 
    async () => {
      ...
      await safeAudit(kv, "reflect", "mem::insight-decay-sweep", dirty.map((i) => i.id), {
        event: "insight.decay",
        decayed,
        softDeleted,
        total: items.length,
