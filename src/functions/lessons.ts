...
export function registerLessonsFunctions(sdk: ISdk, kv: StateKV): void {
  sdk.registerFunction("mem::lesson-save", 
    async (data: {
      ...
      if (project === "*") {
        return { success: false, error: "project '*' is read-only and cannot be used for durable writes" };
      }
      if (project && project.length > 512) {
        return { success: false, error: "project must be at most 512 characters" };
      }
      let originSourceIds = source === "manual" ? [] : (data.sourceIds ?? []);
      let validatedObservationIds: string[] = [];
      let validatedSessionIds: string[] = [];
      const sourceIsObservationProvenance =
        source === "manual" || (data.sources?.length ?? 0) > 0;
      if (
        sourceIsObservationProvenance &&
        ((data.sources?.length ?? 0) > 0 || (data.sourceIds?.length ?? 0) > 0)
      ) {
        if (!project) {
          return {
            success: false,
            error: "project is required when source provenance is provided",
          };
        }
        try {
          const provenance = await validateObservationProvenance(kv, {
            ...
            sourceObservationIds: data.sourceIds,
          });
          validatedObservationIds = provenance.sourceObservationIds;
          validatedSessionIds = provenance.sourceSessionIds;
        } catch (error) {
          return {
            success: false,
      ...
      const existing = (await kv.list<Lesson>(KV.lessons)).find(
        (lesson) =>
          !lesson.deleted &&
          lesson.project === project &&
          lesson.content.trim().toLowerCase() === normalizedContent,
      );
      const fp =
        existing?.id ??
        fingerprintId("lsn", `${project ?? ""}\0${normalizedContent}`);
      ...
      if (existing && !existing.deleted) {
        ...
        existing.sourceIds = [
          ...new Set([...(existing.sourceIds ?? []), ...originSourceIds]),
        ];
      ...
      const lesson: Lesson = {
        ...
        reinforcements: 0,
        source,
        sourceIds: originSourceIds,
        sourceObservationIds: validatedObservationIds,
        sourceSessionIds: validatedSessionIds,
        project,
        tags: data.tags || [],
        createdAt: now,
        updatedAt: now,
  ...
  sdk.registerFunction("mem::lesson-recall", 
    async (data: {
      ...
      for (let i = 0; i < hits.length; i++) {
        ...
        if (data.project && data.project !== "*" && l.project !== data.project) {
          continue;
        }
    ...
    async () => {
      ...
      await Promise.all(
        auditEvents.map((event) =>
          safeAudit(kv, "lesson_strengthen", "mem::lesson-decay-sweep", [event.id], {
            action: event.action,
            actor: "system",
            reason: "decay-sweep",
            before: {
      ...
      await safeAudit(kv, "lesson_strengthen", "mem::lesson-decay-sweep", dirty.map((l) => l.id), {
        event: "lesson.decay.sweep",
        decayed,
        softDeleted,
