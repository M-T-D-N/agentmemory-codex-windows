  ...
  sdk.registerFunction("mem::observe", 
    async (payload: HookPayload) => {
      ...
      if (
        ...
        payload.data !== null &&
        isCodexInternalAmbientText(
          (payload.data as Record<string, unknown>)["prompt"],
        )
      ) {
        return {
          success: true,
      ...
      if (dedupMap) {
        ...
        // DedupMap bounds raw inputs to 500 characters. Hash prompt text
        // first so long prompts sharing a prefix still remain distinct;
        // other hook kinds retain the upstream full-payload fallback.
        const dedupInput =
          payload.hookType === "prompt_submit"
            ? createHash("sha256")
                .update(
                  typeof d["prompt"] === "string"
                    ? d["prompt"]
                    : JSON.stringify(dataIsObject ? d : payload.data),
                )
                .digest("hex")
      ...
      return withKeyedLock(`obs:${payload.sessionId}`, async () => {
        ...
        if (
          ...
          requestedProject &&
          existingSession.project !== requestedProject
        ) {
          return {
            success: false,
