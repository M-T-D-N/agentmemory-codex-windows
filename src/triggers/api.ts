...
function normalizeReadProject(raw: unknown): string | null {
...
export function registerApiTriggers(
  ...
  sdk.registerFunction("api::search",
    async (
      ...
      const project = normalizeReadProject(body.project);
      if (!project) {
        return {
          status_code: 400,
          body: { error: "project is required; use '*' only for a deliberate cross-project read" },
        };
      }
      if (
  ...
  sdk.registerFunction("api::session::exclude",
    async (
      req: ApiRequest<{
        sessionId: string;
        project: string;
      ...
      if (!sessionId || !project || !cwd || !reason) {
        return {
          status_code: 400,
          body: {
            error: "sessionId, project, cwd, and reason are required non-empty strings",
          },
        };
      }
      ...
      const result = await withKeyedLock(sessionLifecycleLockKey(sessionId), async () => {
        ...
        if (existing.project !== project || existing.cwd !== cwd) {
          return {
            status_code: 409,
        ...
        await safeAudit(
          ...
          "api::session::exclude",
          [sessionId],
          { project, reason: captureExclusionReason },
        );
        return {
          status_code: 200,
          body: { success: true, captureExcluded: true, sessionId },
        };
      });
  ...
  sdk.registerTrigger({
    type: "http",
    function_id: "api::session::exclude",
    config: {
      api_path: "/agentmemory/session/exclude",
      http_method: "POST",
      middleware_function_ids: ["middleware::api-auth"],
    },
  });
    ...
    async (req: ApiRequest): Promise<Response> => {
      ...
      const sessions = fetched.filter(
        (session): session is Session =>
          session !== null && !isExcludedCodexAmbientSession(session),
      );
      return {
        status_code: 200,
        body: {
          commit: { ...link, sessionIds: sessions.map((session) => session.id) },
          sessions,
        },
      };
    },
    ...
    async (req: ApiRequest): Promise<Response> => {
      ...
      const visibleSessionIds = new Set(
        (await kv.list<Session>(KV.sessions))
          .filter((session) => !isExcludedCodexAmbientSession(session))
      ...
      const filtered = all
        ...
        .map((commit) => ({
          ...commit,
          sessionIds: (commit.sessionIds ?? []).filter((sessionId) =>
    ...
    async (req: ApiRequest): Promise<Response> => {
      ...
      if (!project) {
        return {
          ...
          body: {
            error:
              "project is required; use '*' only for a deliberate cross-project read",
          },
        };
      }
      const includeExcluded = req.query_params?.["includeExcluded"] === "true";
      const sessionId = asNonEmptyString(req.query_params?.["sessionId"]);
      if (includeExcluded && (project === "*" || !sessionId)) {
        return {
          status_code: 400,
          body: {
            error: "includeExcluded requires an exact project and sessionId",
          },
        };
      }
      const limit = Math.max(
        1,
        Math.min(500, parseOptionalInt(req.query_params?.["limit"]) ?? 100),
      );
      ...
      for (let batch = 0; batch < page.length; batch += 10) {
  ...
  sdk.registerFunction("api::remember",
    async (
      ...
      if (
        ...
        return {
          status_code: 400,
          body: { error: "exact project is required and '*' is read-only" },
        };
      }
      const result = await sdk.trigger({
      ...
      if (
        ...
        return { status_code: 422, body: result };
      }
      return { status_code: 201, body: result };
    },
  );
  ...
  sdk.registerFunction("api::smart-search",
    async (
      ...
      const project = normalizeReadProject(req.body?.project);
      if (!project) {
        return {
          status_code: 400,
          body: { error: "project is required; use '*' only for a deliberate cross-project read" },
  ...
  sdk.registerFunction("api::graph-query",
    async (
      ...
      if (!project) {
        return {
          status_code: 400,
          body: { error: "project is required for graph query" },
        };
      }
      ...
      if (queries !== undefined && (
        ...
        || queries.length < 1
        || queries.length > 8
        || queries.some((query) => typeof query !== "string" || !query.trim() || query.length > 128)
      )) {
        return {
          status_code: 400,
          body: { error: "queries must contain 1-8 non-empty strings up to 128 characters" },
        };
      }
      ...
      try {
        ...
        return {
          ...
          body: {
            error:
              error instanceof Error ? error.message : "Knowledge graph query failed",
          },
        };
      }
    },
  ...
  sdk.registerFunction("api::graph-upsert",
    async (req: ApiRequest): Promise<Response> => {
      const authErr = checkAuth(req, secret);
      if (authErr) return authErr;
      const body = (req.body ?? {}) as unknown as Record<string, unknown>;
      ...
      if (!project || project === "*") {
        return {
          status_code: 400,
          body: { error: "exact project is required for graph upsert" },
        };
      }
      if (!Array.isArray(body.sources) || !Array.isArray(body.nodes)) {
        return {
          status_code: 400,
          body: { error: "sources and nodes must be arrays" },
        };
      }
      if (body.edges !== undefined && !Array.isArray(body.edges)) {
        return { status_code: 400, body: { error: "edges must be an array" } };
      }
      const result = await sdk.trigger({
        function_id: "mem::graph-upsert",
        payload: {
          project,
  ...
  sdk.registerFunction("api::graph-project-purge",
    async (req: ApiRequest): Promise<Response> => {
      ...
      if (!project || project === "*" || !reason) {
        return {
          status_code: 400,
          body: { error: "exact project and non-empty reason are required" },
        };
      }
      if (!Array.isArray(body.nodeIds) || !Array.isArray(body.edgeIds)) {
        return {
          status_code: 400,
          body: { error: "nodeIds and edgeIds must be arrays" },
        };
      }
      const result = await sdk.trigger({
        function_id: "mem::graph-project-purge",
        payload: {
          project,
  ...
  sdk.registerTrigger({
    type: "http",
    function_id: "api::graph-project-purge",
    config: {
      api_path: "/agentmemory/graph/project-purge",
      http_method: "POST",
    },
  });
  ...
  sdk.registerFunction("api::graph-extract",
    async (req: ApiRequest<{
      ...
      if ((project && !sessionId) || (!project && sessionId)) {
        return {
          status_code: 400,
          body: { error: "project and sessionId must be supplied together" },
        };
      }
      try {
        const result = await sdk.trigger({
  ...
  sdk.registerFunction("api::routine-create",
    async (req: ApiRequest<{ name?: string; steps?: unknown }>): Promise<Response> => {
      const authErr = checkAuth(req, secret);
      if (authErr) return authErr;
  ...
  sdk.registerFunction("api::verify",  async (req: ApiRequest) => {
    ...
    if (!project) return { status_code: 400, body: { error: "project is required" } };
    const result = await sdk.trigger({ function_id: "mem::verify", payload: { id: body.id, project } });
    return { status_code: 200, body: result };
  });
  ...
  sdk.registerFunction("api::lesson-save",  async (req: ApiRequest) => {
    ...
    if (body.sourceIds !== undefined && !Array.isArray(body.sourceIds)) {
      return { status_code: 400, body: { error: "sourceIds must be an array" } };
    }
  ...
  sdk.registerFunction("api::lesson-list",  async (req: ApiRequest) => {
    ...
    if (denied) return denied;
    const params = req.query_params || {};
    const project = normalizeReadProject(params.project);
    if (!project) {
      return {
        status_code: 400,
        body: { error: "project is required; use '*' only for a deliberate cross-project read" },
  ...
  sdk.registerFunction("api::lesson-search",  async (req: ApiRequest) => {
    ...
    const result = await sdk.trigger({ function_id: "mem::lesson-recall", payload: {
      query: body.query,
      project,
