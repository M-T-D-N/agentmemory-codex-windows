...
} from "./functions/semantic-graph-backlog.js";
...
async function main() {
  ...
  const localQwenGraphOnly = config.provider.provider === "local-qwen";
  if (localQwenGraphOnly) {
    const capabilities = new Set(
      (getEnvVar("AGENTMEMORY_PROVIDER_CAPABILITIES") || "graph")
        .split(",")
        .map((value) => value.trim().toLowerCase())
        .filter(Boolean),
    );
    if (capabilities.size !== 1 || !capabilities.has("graph")) {
      throw new Error(
        "local-qwen supports exactly AGENTMEMORY_PROVIDER_CAPABILITIES=graph",
      );
    }
  }
  const generalProviderConfig = localQwenGraphOnly
    ? { provider: "noop" as const, model: "noop", maxTokens: 0 }
    : config.provider;
  const provider =
  ...
  if (localQwenGraphOnly && graphProvider.probe) {
    try {
      graphProviderRuntime = await graphProvider.probe();
    } catch (error) {
      bootLog(
        `Local Qwen capability probe deferred: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  if (localQwenGraphOnly) {
    bootLog("Provider capabilities: graph=local-qwen; all other LLM features=noop");
    if (graphProviderRuntime) {
      bootLog(
        `Local Qwen runtime: model=${graphProviderRuntime.model}, context=${graphProviderRuntime.contextTokens}, input-cap=${graphProviderRuntime.maxInputTokens}`,
      );
    }
  }
  ...
  if (process.env.LESSON_DECAY_ENABLED !== "false") {
    ...
    void runLessonDecay();
    const lessonDecayTimer = setInterval(runLessonDecay, 86400000);
  ...
  if (process.env.INSIGHT_DECAY_ENABLED !== "false") {
    ...
    void runInsightDecay();
    const insightDecayTimer = setInterval(runInsightDecay, 86400000);
    insightDecayTimer.unref();
  }

  if (process.env.RETENTION_SCORING_ENABLED !== "false") {
    const retentionIntervalMs = Math.max(
      60000,
      parseInt(process.env.RETENTION_SCORING_INTERVAL_MS || "3600000", 10) ||
        3600000,
    );
    const runRetentionScoring = async () => {
      try {
        await sdk.trigger({ function_id: "mem::retention-score", payload: {} });
      } catch {}
    };
    void runRetentionScoring();
    const retentionScoringTimer = setInterval(
      runRetentionScoring,
      retentionIntervalMs,
    );
    retentionScoringTimer.unref();
    bootLog(
      `Retention scoring: enabled (startup + every ${retentionIntervalMs / 60000}m; eviction remains explicit)`,
    );
  }
