...
function getFallbackKv(): InMemoryKV {
  fallbackKv ??= new InMemoryKV(getStandalonePersistPath());
  return fallbackKv;
}
  ...
  if (result && Array.isArray(result.content)) {
    ...
    for (const block of result.content) {
      ...
      try {
        ...
        // Non-JSON text is valid MCP content. HTTP status remains authoritative.
      }
    }
    return response;
  }
...
export async function handleToolCall(
  ...
  if (handle.mode === "proxy") {
    try {
      ...
      invalidateHandle();
      // Once the server passed livez, never reinterpret a failed official
      // write/read as permission to create or read a second local store.
      throw err;
    }
  }
  ...
  if (isForceProxyEnabled()) {
    throw new Error(
      `Official AgentMemory server unavailable at ${getAgentMemoryBaseUrl()}; local fallback is disabled`,
    );
  }
  return handleLocal(validated, kvInstance ?? getFallbackKv());
}
...
export async function handleToolsList(): Promise<{ tools: unknown[] }> {
  ...
  if (handle.mode === "proxy") {
    try {
      ...
      if (isForceProxyEnabled()) {
        throw new Error("AgentMemory server returned an invalid tools/list response");
      }
      process.stderr.write(
      ...
      process.stderr.write(
        `[@agentmemory/mcp] tools/list proxy failed: ${err instanceof Error ? err.message : String(err)}${isForceProxyEnabled() ? "" : "; falling back to local list"}\n`,
      );
      invalidateHandle();
      if (isForceProxyEnabled()) throw err;
    }
  }
  if (isForceProxyEnabled()) {
    throw new Error(
      `Official AgentMemory server unavailable at ${getAgentMemoryBaseUrl()}; local tools/list fallback is disabled`,
    );
  }
...
export function startAgentMemoryStreamableHttpServer(
  options: Omit<StreamableHttpOptions, "handler">,
): Promise<StreamableHttpServer> {
  return startStreamableHttpServer({ ...options, handler: handleMcpRequest });
}

if (process.env["AGENTMEMORY_MCP_EMBEDDED"] !== "true") {
  const transport = createStdioTransport(handleMcpRequest);
  process.stderr.write(
