...
function parseStrictCsvList(value: unknown): string[] | null {
        ...
        switch (name) {
          case "memory_recall": {
            ...
            if (!project) {
              return {
                status_code: 400,
                body: { error: "project is required for memory_recall" },
              };
            }
            const format =
          ...
          case "memory_save": {
            ...
            if (!project || project === "*") {
              return {
                ...
                body: {
                  error:
                    "exact project is required for memory_save; '*' is read-only",
                },
              };
            }
          ...
          case "memory_graph_purge": {
            ...
            if (!project || project === "*" || !reason) {
              return {
                ...
                body: {
                  error:
                    "exact project and non-empty reason are required for memory_graph_purge",
                },
              };
            }
            if (!Array.isArray(args.nodeIds) || !Array.isArray(args.edgeIds)) {
              return {
                status_code: 400,
          ...
          case "memory_graph_query": {
            try {
              ...
              if (!project) {
                return {
                  status_code: 400,
                  body: { error: "project is required for memory_graph_query" },
                };
              }
          ...
          case "memory_verify": {
            ...
            const verifyResult = await sdk.trigger({ function_id: "mem::verify", payload: { id: args.id, project } });
            return mcpToolResult(
              verifyResult,
              Boolean(
                verifyResult &&
                typeof verifyResult === "object" &&
                (verifyResult as { success?: boolean }).success === false,
              ),
            );
          }
          ...
          case "memory_lesson_save": {
            ...
            if (!project || project === "*") {
              return {
                ...
                body: {
                  error:
                    "exact project is required for memory_lesson_save; '*' is read-only",
                },
              };
            }
            const lessonTags = typeof args.tags === "string" && args.tags.trim()
              ? args.tags.split(",").map((t: string) => t.trim()).filter(Boolean)
              : [];
            const sourceIds = parseStrictCsvList(args.sourceIds);
            if (sourceIds === null) {
              return {
                status_code: 400,
                body: { error: "sourceIds must contain only non-empty strings" },
              };
            }
            ...
            if (
              lessonSaveResult &&
              typeof lessonSaveResult === "object" &&
              (lessonSaveResult as { success?: boolean }).success === false
            ) {
              return mcpToolResult(lessonSaveResult, true);
            }
            return mcpToolResult(lessonSaveResult);
          }
          ...
          case "memory_lesson_recall": {
            ...
            if (!project) {
              return {
                status_code: 400,
                body: { error: "project is required for memory_lesson_recall" },
              };
            }
            ...
            return {
              ...
              body: {
                content: [
                  {
                    type: "text",
                    text: JSON.stringify({ commit: visibleLink, sessions }, null, 2),
                  },
                ],
              },
            };
            ...
            const filtered = (all as Array<{ branch?: string; repo?: string; linkedAt?: string }>)
              ...
              .map((commit) => {
                const sessionIds = Array.isArray(
                  (commit as { sessionIds?: unknown }).sessionIds,
                )
                  ? ((commit as { sessionIds: string[] }).sessionIds ?? []).filter(
                      (sessionId) => visibleSessionIds.has(sessionId),
                    )
                  : [];
                return { ...commit, sessionIds };
              });
            return {
              status_code: 200,
          ...
          const filtered = summaries
            .filter(
              (summary) =>
                ...
                visibleSessionIds.has(summary.sessionId),
            )
            .sort(
              (a, b) =>
      ...
      try {
        switch (promptName) {
          ...
          case "session_handoff": {
            ...
            if (!session || isExcludedCodexAmbientSession(session)) {
