...
export async function processLine(
  ...
  const response = await processJsonRpcRequest(parsed, handler, writeErr);
  if (response) writeOut(response);
}

// Shared by stdio and Streamable HTTP so both transports preserve one MCP
// request contract. A null result means the input was a notification (or a
// malformed notification-shaped message) and must not receive a response.
export async function processJsonRpcRequest(
  parsed: unknown,
  handler: RequestHandler,
  writeErr: (msg: string) => void = (msg) => process.stderr.write(msg),
