...
function detectProvider(env: Record<string, string>): ProviderConfig {
  ...
  if ((env["AGENTMEMORY_PROVIDER"] || "").toLowerCase() === "local-qwen") {
    return {
      provider: "local-qwen",
      model: env["AGENTMEMORY_LOCAL_QWEN_MODEL"] || "auto",
      maxTokens: safeParseInt(
        env["AGENTMEMORY_LOCAL_QWEN_MAX_OUTPUT_TOKENS"],
        2048,
      ),
      baseURL:
        env["AGENTMEMORY_LOCAL_QWEN_BASE_URL"] ||
        "http://127.0.0.1:8000",
    };
  }
...
export function detectLlmProviderKind(): "llm" | "noop" {
  const env = getMergedEnv();
  if (
    (env["AGENTMEMORY_PROVIDER"] || "").toLowerCase() === "local-qwen" ||
...
export function isSummaryEnabled(): boolean {
  const env = getMergedEnv();
  const explicit = env["AGENTMEMORY_SUMMARY_ENABLED"];
  if (explicit === "false" || explicit === "0") return false;
  if (explicit === "true" || explicit === "1") return true;
  return (env["AGENTMEMORY_PROVIDER"] || "").toLowerCase() !== "local-qwen";
}
