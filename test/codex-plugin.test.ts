...
describe("Windows/Codex downstream installation boundary", () => {
  it("does not advertise the generic compatibility plugin as the supported marketplace install", () => {
    expect(existsSync(join(repoRoot, ".codex-plugin/marketplace.json"))).toBe(false);
  });
});
