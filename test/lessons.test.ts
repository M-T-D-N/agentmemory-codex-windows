...
describe("Lessons", () => {
  ...
  describe("mem::lesson-save", () => {
    ...
    it("sets crystal source and sourceIds when provided", async () => {
      ...
      expect(result.lesson.sourceObservationIds).toEqual([]);
      expect(result.lesson.confidence).toBe(0.6);
    });

    it("validates and preserves official observation provenance", async () => {
      await kv.set("mem:sessions", "ses_source", {
        id: "ses_source",
        project: "/test",
        cwd: "/test",
      ...
      await kv.set("mem:obs:ses_source", "obs_source", {
        id: "obs_source",
        ...
        title: "source",
        facts: [],
        narrative: "verified lesson source",
        concepts: [],
        files: [],
        importance: 8,
      });
      ...
      const result = (await sdk.trigger("mem::lesson-save", {
        content: "Use verified citations",
        project: "/test",
        sourceIds: ["obs_source"],
      })) as { success: boolean; lesson: Lesson };

      expect(result.success).toBe(true);
      expect(result.lesson.sourceIds).toEqual([]);
      expect(result.lesson.sourceObservationIds).toEqual(["obs_source"]);
      expect(result.lesson.sourceSessionIds).toEqual(["ses_source"]);
    });

    it("rejects cross-project manual provenance without writing", async () => {
      ...
      await kv.set("mem:obs:ses_other", "obs_other", {
        ...
        title: "other",
        facts: [],
        narrative: "foreign lesson source",
        concepts: [],
        files: [],
        importance: 8,
      });
  ...
  describe("mem::lesson-recall", () => {
    ...
    it("treats project '*' as an explicit cross-project read", async () => {
      const result = (await sdk.trigger("mem::lesson-recall", {
        query: "database typescript",
        project: "*",
      })) as { lessons: Lesson[] };

      expect(new Set(result.lessons.map((lesson) => lesson.project))).toEqual(
        new Set(["/app", "/other"]),
      );
    });
