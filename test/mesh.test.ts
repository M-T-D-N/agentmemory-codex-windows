...
describe("Mesh Functions", () => {
  ...
  describe("mesh-receive expanded scopes", () => {
    ...
    it("uses graph edge updatedAt for provenance merge LWW", async () => {
      ...
      await kv.set("mem:graph:edges", original.id, original);
      const updated: GraphEdge = {
        ...original,
        weight: 0.9,
        sourceObservationIds: ["obs_old", "obs_new"],
        updatedAt: "2026-03-02T00:00:00Z",
      };
      ...
      expect(result).toEqual(expect.objectContaining({ success: true, accepted: 1 }));
      const stored = await kv.get<GraphEdge>("mem:graph:edges", original.id);
      expect(stored).toMatchObject({ weight: 0.9, sourceObservationIds: ["obs_old", "obs_new"] });
    });
