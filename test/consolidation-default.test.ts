...
describe("isConsolidationEnabled default behavior", () => {
  ...
  it("keeps consolidation off by default for the graph-scoped local-Qwen provider", async () => {
    writeEnv("AGENTMEMORY_PROVIDER=local-qwen");
    const cfg = await freshConfig();
    expect(cfg.isConsolidationEnabled()).toBe(false);
    expect(cfg.isSummaryEnabled()).toBe(false);
  });
