...
describe("Reflect", () => {
  ...
  beforeEach(async () => {
    ...
    await kv.set("mem:sessions", "sess_app", {
      id: "sess_app",
      project: "/app",
      status: "completed",
  ...
  describe("mem::reflect", () => {
    it("returns empty when no graph nodes or memories exist", async () => {
      const result = (await sdk.trigger("mem::reflect", { project: "/app" })) as {
        success: boolean;
        newInsights: number;
    ...
    it("deduplicates insights by fingerprint", async () => {
      ...
      await sdk.trigger("mem::reflect", { project: "/app" });
  ...
  describe("mem::insight-decay-sweep", () => {
    ...
    it("does not invert a successful decay when audit persistence fails", async () => {
      await kv.set("mem:insights", "ins_audit_failure", {
        id: "ins_audit_failure", title: "Audit failure", content: "Old insight", confidence: 0.8,
        reinforcements: 1, sourceConceptCluster: [], sourceMemoryIds: [],
        sourceLessonIds: [], sourceCrystalIds: [], tags: [],
        createdAt: new Date(Date.now() - 21 * 86400000).toISOString(),
        updatedAt: new Date(Date.now() - 21 * 86400000).toISOString(),
      ...
      kv.set = async <T>(scope: string, key: string, data: T): Promise<T> => {
        if (scope === "mem:audit") throw new Error("audit unavailable");
        return originalSet(scope, key, data);
      };

      const result = (await sdk.trigger("mem::insight-decay-sweep", {})) as {
        success: boolean;
        decayed: number;
      };
      expect(result).toMatchObject({ success: true, decayed: 1 });
      const after = await kv.get<Insight>("mem:insights", "ins_audit_failure");
      expect(after!.confidence).toBeLessThan(0.8);
    });
  });
});
