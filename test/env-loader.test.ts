...
describe("loadEnvFile", () => {
  ...
  it("loads local Qwen as an auto-discovered graph provider without a key", async () => {
    writeEnv([
      "AGENTMEMORY_PROVIDER=local-qwen",
      "AGENTMEMORY_LOCAL_QWEN_BASE_URL=http://127.0.0.1:8000",
      "AGENTMEMORY_LOCAL_QWEN_MODEL=auto",
      "AGENTMEMORY_LOCAL_QWEN_MAX_OUTPUT_TOKENS=2048",
    ].join("\n"));
    const cfg = await freshConfig();
    expect(cfg.loadConfig().provider).toEqual({
      provider: "local-qwen",
      model: "auto",
      maxTokens: 2048,
