...
describe("MCP Resources", () => {
  ...
  it("hides internal Codex sessions from memory_sessions", async () => {
    ...
    await kv.set("mem:sessions", visible.id, visible);
    await kv.set("mem:sessions", "ses_internal", {
      ...visible,
      id: "ses_internal",
      captureExcluded: true,
    ...
    expect(result.status_code).toBe(200);
    expect(sessions.map((session: Session) => session.id)).toEqual(["ses_visible"]);
  });

  it("hides internal session IDs from MCP commit views", async () => {
    const visible: Session = {
      id: "ses_visible",
      project: "/test",
      cwd: "/test",
    ...
    await kv.set("mem:commits", "abc123", {
      sha: "abc123",
      sessionIds: ["ses_visible", "ses_internal"],
      linkedAt: "2026-08-20T00:00:00Z",
    });
    const fn = sdk.getFunction("mcp::tools::call")!;

    const lookupResponse = (await fn(
      makeReq({ name: "memory_commit_lookup", arguments: { sha: "abc123" } }),
    )) as { body: { content: Array<{ text: string }> } };
    const listResponse = (await fn(
      makeReq({ name: "memory_commits", arguments: {} }),
    )) as { body: { content: Array<{ text: string }> } };
    const lookup = JSON.parse(lookupResponse.body.content[0].text);
    const list = JSON.parse(listResponse.body.content[0].text);

    expect(lookup.commit.sessionIds).toEqual(["ses_visible"]);
    expect(lookup.sessions.map((session: Session) => session.id)).toEqual([
      "ses_visible",
    ]);
    expect(list.commits[0].sessionIds).toEqual(["ses_visible"]);
  });
  ...
  it("reads agentmemory://project/{name}/recent with sorted summaries", async () => {
    ...
    for (const summary of summaries) {
      await kv.set("mem:sessions", summary.sessionId, {
    ...
    await kv.set("mem:summaries", "ses_internal", {
      ...summaries[1],
    ...
    expect(data.some((summary: SessionSummary) => summary.sessionId === "ses_internal")).toBe(false);
  });
