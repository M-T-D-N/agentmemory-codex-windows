...
describe("event::session::stopped consolidation fan-out", () => {
  beforeEach(() => {
    ...
    vi.mocked(isSummaryEnabled).mockReturnValue(true);
  ...
  it("does not call the provider-backed summary path when summary is disabled", async () => {
    vi.mocked(isSummaryEnabled).mockReturnValue(false);
    const { sdk, handlers, trigger } = mockSdk();
    registerEventTriggers(sdk as never, mockKV() as never);
    ...
    expect(result).toEqual({ success: false, error: "summary_disabled" });
    expect(functionIds(trigger)).not.toContain("mem::summarize");
  });

  it("sends a bounded exact-project graph batch and records pending state", async () => {
    vi.mocked(isSummaryEnabled).mockReturnValue(false);
    vi.mocked(isConsolidationEnabled).mockReturnValue(false);
    ...
    const observations = Array.from({ length: 12 }, (_, index) => ({
      ...
      concepts: [],
      files: [],
      createdAt: `2026-08-23T00:${String(index).padStart(2, "0")}:00.000Z`,
    }));
    const kv = {
      ...mockKV(),
      get: vi.fn(async (scope: string, key: string) =>
        scope === "mem:sessions" && key === "ses_1"
          ? { id: "ses_1", project: "/project-a" }
          : null),
      list: vi.fn(async (scope: string) =>
        scope === "mem:obs:ses_1" ? observations : []),
      update: vi.fn(async () => {}),
    };
    const { sdk, handlers, trigger } = mockSdk();
    ...
    const graphCall = trigger.mock.calls.find(
      (call) => (call[0] as { function_id: string }).function_id === "mem::graph-extract",
    );
    ...
    const graphPayload = (graphCall?.[0] as { payload: { observations: unknown[] } }).payload;
    expect(graphPayload.observations).toHaveLength(10);
    expect(graphPayload.observations[0]).toMatchObject({ id: "obs_1" });
    expect(kv.update).toHaveBeenCalledWith(
      "mem:sessions",
      "ses_1",
