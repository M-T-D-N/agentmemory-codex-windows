...
describe("Verify Function", () => {
  ...
  it("verifies a memory with citation chain", async () => {
    ...
    const result = (await sdk.trigger("mem::verify", { id: "mem_1", project: "/test/project" })) as {
      success: boolean;
      type: string;
      citations: Array<{
  ...
  it("verifies a memory with no source observations", async () => {
    ...
    const result = (await sdk.trigger("mem::verify", { id: "mem_2", project: "*" })) as {
      success: boolean;
      type: string;
      citationCount: number;
      citations: unknown[];
  ...
  it("returns memory info with supersede chain", async () => {
    ...
    const result = (await sdk.trigger("mem::verify", { id: "mem_v2", project: "*" })) as {
      success: boolean;
      memory: {
        id: string;
        version: number;
