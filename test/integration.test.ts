...
describe("agentmemory integration", () => {
  ...
  describe("session lifecycle", () => {
    ...
    it("lists sessions including the new one", async () => {
      const res = await fetch(url(`/agentmemory/sessions?project=${encodeURIComponent(PROJECT)}`));
      expect(res.status).toBe(200);
      const body = (await json(res)) as {
  ...
  describe("observations", () => {
    ...
    it("lists observations for the session", async () => {
      const res = await fetch(
        url(`/agentmemory/observations?project=${encodeURIComponent(PROJECT)}&sessionId=${OBS_SESSION}`),
      );
      expect(res.status).toBe(200);
      const body = (await json(res)) as {
  ...
  describe("search", () => {
    it("searches observations", async () => {
      const res = await fetch(url("/agentmemory/search"), {
        method: "POST",
        headers: authHeaders(),
        body: JSON.stringify({ query: "auth", project: PROJECT, limit: 5 }),
      });
      expect(res.status).toBe(200);
