...
describe("buildSyntheticCompression", () => {
  ...
  it("preserves raw user and final assistant text without separators", async () => {
    ...
    const prompt = "user ".repeat(140);
    const response = "assistant ".repeat(100);
    const user = buildSyntheticCompression({
      id: "obs_user",
      sessionId: "ses_1",
      timestamp: new Date().toISOString(),
    ...
    expect(user.narrative).toBe(prompt);
    expect(assistant.narrative).toBe(response);
  });
