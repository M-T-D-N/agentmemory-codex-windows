...
describe("mem::search", () => {
  ...
  it("returns full format by default", async () => {
    ...
    })) as { format: string; results: Array<{ observation: CompressedObservation; project?: string }> };
  ...
  it("returns compact format when requested", async () => {
    const result = (await sdk.trigger("mem::search", {
      query: "auth",
      format: "compact",
    })) as { format: string; results: Array<{ obsId: string; title: string; project?: string }> };
  ...
  it("treats project '*' as an explicit cross-project read", async () => {
    const result = (await sdk.trigger("mem::search", {
      query: "auth middleware",
      project: "*",
    })) as { results: Array<{ observation: CompressedObservation }> };

    expect(result.results.map((entry) => entry.observation.id)).toContain(
      "obs_a",
    );
  });
  ...
  it("rejects a stale superseded memory even when its index entry remains", async () => {
    const stale: Memory = {
      id: "mem_stale",
      createdAt: "2026-02-01T00:00:00Z",
      updatedAt: "2026-02-02T00:00:00Z",
      type: "fact",
      title: "Obsolete pineapple rule",
      content: "obsolete pineapple rule retained only for provenance",
      concepts: ["pineapple"],
      files: [],
      sessionIds: [],
      strength: 7,
    ...
    await kv.set(KV.memories, stale.id, stale);
    getSearchIndex().add(memoryToObservation(stale));

    const result = (await sdk.trigger("mem::search", {
      query: "obsolete pineapple rule",
      project: "demo",
    })) as { results: Array<{ observation: CompressedObservation }> };

    expect(
      result.results.some((entry) => entry.observation.id === stale.id),
    ).toBe(false);
  });

  it("does not surface observations from capture-excluded sessions", async () => {
    ...
    const hiddenObservation: CompressedObservation = {
      id: "obs_hidden",
      sessionId: hiddenSession.id,
      timestamp: hiddenSession.startedAt,
      type: "conversation",
      title: "Codex internal ambient fragment",
      facts: [],
      narrative: "unique-internal-ambient-fragment",
      concepts: [],
      files: [],
      importance: 5,
    };
    await kv.set(KV.sessions, hiddenSession.id, hiddenSession);
    await kv.set(
      KV.observations(hiddenSession.id),
      hiddenObservation.id,
      hiddenObservation,
    );
    await rebuildIndex(kv as never);

    const result = (await sdk.trigger("mem::search", {
      query: "unique-internal-ambient-fragment",
      project: "demo",
    })) as { results: Array<{ observation: CompressedObservation }> };
