...
describe("IndexPersistence", () => {
  ...
  it("writes large index shards sequentially to protect foreground state calls", async () => {
    const bm25 = new SearchIndex();
    bm25.add(
      makeObs({
        id: "obs_sequential",
        title: "sequential persisted snapshot ".repeat(40),
        narrative: "foreground MCP protection ".repeat(40),
      }),
    );
    let activeShardWrites = 0;
    let maxActiveShardWrites = 0;
    const guardedKv = {
      ...kv,
      set: vi.fn(async <T>(scope: string, key: string, data: T): Promise<T> => {
        if (scope.includes(":gen_sequential:")) {
          activeShardWrites += 1;
          maxActiveShardWrites = Math.max(
            maxActiveShardWrites,
            activeShardWrites,
          );
    ...
    await new IndexPersistence(guardedKv as never, bm25, null, {
      shardChars: 80,
      createGeneration: () => "gen_sequential",
    }).save();

    const manifest = await getBm25Manifest(kv);
    expect(manifest.shards.length).toBeGreaterThan(1);
    expect(maxActiveShardWrites).toBe(1);
  });

  it("serializes overlapping scheduled and foreground snapshot saves", async () => {
    const bm25 = makeBm25("obs_serialized_save", "serialized snapshot save");
    let activeManifestWrites = 0;
    let maxActiveManifestWrites = 0;
    const guardedKv = {
      ...kv,
      set: vi.fn(async <T>(scope: string, key: string, data: T): Promise<T> => {
        if (scope === BM25_SCOPE && key === BM25_MANIFEST_KEY) {
          activeManifestWrites += 1;
          maxActiveManifestWrites = Math.max(
            maxActiveManifestWrites,
            activeManifestWrites,
          );
          await Promise.resolve();
          const result = await kv.set(scope, key, data);
          activeManifestWrites -= 1;
    ...
    const persistence = new IndexPersistence(guardedKv as never, bm25, null, {
      shardChars: 80,
      createGeneration: () => `gen_queued_${++generation}`,
    });
