...
describe("mem::consolidate — cross-project existingMatch guard", () => {
  ...
  it("excludes internal sessions and strips ambient UI blocks before consolidation", async () => {
    const sdk = makeMockSdk();
    const kv = makeMockKV();
    const provider = makeProvider("visible memory");

    const visibleSession = makeSession("sess_visible", "api");
    const excludedSession = {
      ...makeSession("sess_internal", "api"),
      captureExcluded: true,
      captureExclusionReason: "codex_internal",
    };
    await kv.set(KV.sessions, visibleSession.id, visibleSession);
    await kv.set(KV.sessions, excludedSession.id, excludedSession);
    for (let i = 0; i < 3; i++) {
      const observation = makeObs(`obs_visible_${i}`, visibleSession.id, "auth");
      observation.narrative =
        `visible ${i} ` +
        '<context source="ambient-ui-state">do-not-consolidate</context>';
      await kv.set(KV.observations(visibleSession.id), observation.id, observation);
    }
    for (let i = 0; i < 3; i++) {
      const observation = makeObs(`obs_internal_${i}`, excludedSession.id, "auth");
      observation.narrative = "internal-session-marker";
      await kv.set(KV.observations(excludedSession.id), observation.id, observation);
    }
    ...
    const prompt = vi.mocked(provider.compress).mock.calls[0][1];
    expect(prompt).toContain("visible 0");
    expect(prompt).not.toContain("do-not-consolidate");
    expect(prompt).not.toContain("internal-session-marker");
  });
});
