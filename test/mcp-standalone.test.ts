...
describe("handleToolCall", () => {
  ...
  const originalForceProxy = process.env["AGENTMEMORY_FORCE_PROXY"];
  ...
  afterEach(() => {
    if (originalForceProxy === undefined) {
      delete process.env["AGENTMEMORY_FORCE_PROXY"];
    } else {
      process.env["AGENTMEMORY_FORCE_PROXY"] = originalForceProxy;
    }
    (globalThis as { fetch: typeof fetch }).fetch = originalFetch;
    resetHandleForTests();
  });

  it("forced proxy failure never falls back to local persistence", async () => {
    process.env["AGENTMEMORY_FORCE_PROXY"] = "true";
    resetHandleForTests();
    const proxyFailure = vi.fn(async () => {
      throw new Error("proxy unavailable");
    });
    (globalThis as { fetch: typeof fetch }).fetch = proxyFailure as unknown as typeof fetch;
    const kv = new InMemoryKV();

    await expect(
      handleToolCall("memory_save", { content: "must never be local" }, kv),
    ).rejects.toThrow("proxy unavailable");
    expect(await kv.list("mem:memories")).toEqual([]);
  });

  it("forced proxy forwards exact arguments and marks structured failures", async () => {
    process.env["AGENTMEMORY_FORCE_PROXY"] = "true";
    resetHandleForTests();
    const proxyFetch = vi.fn(async (_url: unknown, init?: RequestInit) =>
      new Response(
        JSON.stringify({
          content: [
            {
              type: "text",
              text: JSON.stringify({ success: false, error: "remote failure" }),
            },
          ],
        }),
    ...
    const request = proxyFetch.mock.calls[0][1];
  ...
  it("forced tools/list rejects an invalid remote response", async () => {
    process.env["AGENTMEMORY_FORCE_PROXY"] = "true";
    resetHandleForTests();
    const invalidToolsList = vi.fn(async () =>
      new Response(JSON.stringify({ tools: "not-an-array" }), {
        status: 200,
        headers: { "content-type": "application/json" },
      }),
    );
    (globalThis as { fetch: typeof fetch }).fetch = invalidToolsList as unknown as typeof fetch;
  ...
  it("memory_sessions honours the limit arg (#139)", async () => {
    ...
    expect(parsed.sessions.some((session: { id: string }) => session.id === "ses_internal")).toBe(false);
  });
