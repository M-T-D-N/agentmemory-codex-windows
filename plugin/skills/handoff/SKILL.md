...
`memory_recall { "project": "my-project", "query": "<session top concepts>", "limit": 10 }`.
...
2. Resolve the exact registered project and call `memory_sessions` with it. Pick
   the most recent session whose normalized `cwd`
   ...
   No match: report that the project has no matching session. Do not silently
