...
| `memory_recall` | yes | `query`*: string, `project`*: string, `limit`: number, `format`: string, `token_budget`: number | Search past observations and memories inside an explicit project. Pass '*' only for a deliberate cross-project read. |
...
| `memory_smart_search` | yes | `query`*: string, `project`*: string, `expandIds`: string, `limit`: number | Hybrid semantic+keyword search with progressive disclosure inside an explicit project. Pass '*' only for a deliberate cross-project read. |
