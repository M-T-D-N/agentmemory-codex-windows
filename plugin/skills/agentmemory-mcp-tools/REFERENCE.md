...
agentmemory exposes 56 MCP tools. 8 are in the lean core set (`--tools core` or `AGENTMEMORY_TOOLS=core`); the rest load with `--tools all` (default).
...
| `memory_graph_upsert` |  | `project`*: string, `sources`*: array, `nodes`*: array, `edges`: array | Durably upsert Codex-authored zero-LLM graph nodes and relationships into AgentMemory's official graph store with exact source provenance. |
...
| `memory_save` | yes | `content`*: string, `type`: string, `concepts`: string, `files`: string, `project`*: string, `agentId`: string | Save a project-scoped durable insight, decision, or pattern with optional official observation provenance. |
