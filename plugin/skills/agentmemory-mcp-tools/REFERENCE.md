...
| `memory_consolidate` | yes | `tier`: string | Run the LLM-backed 4-tier memory consolidation pipeline. Disabled in zero-LLM/noop mode; use curated memory/lesson/graph writes instead. |
...
| `memory_graph_purge` |  | `project`*: string, `nodeIds`*: array, `edgeIds`*: array, `reason`*: string | Physically delete one explicit project's complete bounded live graph after an exact node/edge inventory. Preserves sessions, observations, memories, and lessons; refuses partial or oversized snapshots. |
| `memory_graph_query` |  | `startNodeId`: string, `nodeType`: string, `maxDepth`: number, `query`: string, `project`*: string, `limit`: number, `offset`: number, `edgeLimit`: number, `edgeOffset`: number | Query AgentMemory's stored knowledge graph inside an explicit project. Use '*' only for a deliberate cross-project read. Automatic graph extraction is not required. |
| `memory_graph_upsert` |  | `project`*: string, `sources`*: array, `sharedSources`: boolean, `nodes`*: array, `edges`: array | Durably upsert Codex-authored zero-LLM graph nodes and relationships into AgentMemory's official graph store with exact source provenance. |
...
| `memory_lesson_delete` |  | `lessonId`*: string, `project`: string | Soft-delete a lesson by id. Deleted lessons are excluded from recall and list; re-saving the same content creates a fresh lesson. |
...
| `memory_lesson_save` | yes | `content`*: string, `context`: string, `confidence`: number, `project`*: string, `tags`: string, `sourceIds`: string, `sources`: array | Save a lesson learned from this session. Lessons have confidence scores that strengthen when reinforced and decay when not used. Duplicate content auto-strengthens the existing lesson. |
...
| `memory_recall` | yes | `query`*: string, `agentId`: string, `project`*: string, `limit`: number, `format`: string, `token_budget`: number, `trackAccess`: boolean | Search past observations and memories inside an explicit project. Pass '*' only for a deliberate cross-project read. |
...
| `memory_save` | yes | `content`*: string, `sourceObservationIds`: array, `type`: string, `concepts`: string, `files`: string, `project`*: string, `agentId`: string | Save a project-scoped durable insight, decision, or pattern with optional official observation provenance. |
...
| `memory_sessions` | yes | `project`*: string, `sessionId`: string, `agentId`: string, `includeExcluded`: boolean, `limit`: integer, `offset`: integer | List recent sessions in an explicit project, newest first. Use '*' only for deliberate cross-project reads; follow nextOffset for more. |
