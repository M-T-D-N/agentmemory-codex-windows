...
| `memory_graph_query` |  | `startNodeId`: string, `nodeType`: string, `maxDepth`: number, `query`: string, `project`*: string, `limit`: number, `offset`: number | Query AgentMemory's stored knowledge graph inside an explicit project. Use '*' only for a deliberate cross-project read. Automatic graph extraction is not required. |
