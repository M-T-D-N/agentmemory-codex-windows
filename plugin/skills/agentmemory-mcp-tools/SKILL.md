...
1. `memory_save` with the exact registered `project`, `content` (the insight), `concepts` (comma-separated keywords), and `files` (comma-separated paths).
2. `memory_smart_search` with the same exact `project`, `query`, and `limit` to retrieve it later. This runs hybrid BM25 plus vector plus graph-expanded search. Use `project: "*"` only for a deliberate cross-project read.
...
4. Keep writes exact-project scoped. Never use `*` for a durable write.
