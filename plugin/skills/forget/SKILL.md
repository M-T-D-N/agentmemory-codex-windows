...
```json
memory_smart_search { "project": "my-project", "query": "old api key in config", "limit": 20 }
```
...
1. Resolve the exact registered project, then search with
   `memory_smart_search`, `project`, the user's text as `query`, and `limit: 20`.
