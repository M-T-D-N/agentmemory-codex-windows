...
```json
memory_sessions { "project": "app", "limit": 30 }
```
...
```json
memory_recall { "project": "app", "query": "jwt-refresh-rotation auth-flow", "limit": 3 }
```
...
Treat as `last 3`. Call `memory_sessions { "project": "app", "limit": 3 }`, group by date, same format.
