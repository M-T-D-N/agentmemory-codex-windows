...
```json
memory_sessions { "project": "my-project", "limit": 30 }
```

Then per surviving session: `memory_recall { "project": "my-project", "query": "<top concepts>", "limit": 3 }`.
...
2. Resolve the exact registered project and call `memory_sessions` with that
   `project`. Apply the window and sort by `startedAt` descending.
...
- Sessions came from the exact registered project.
