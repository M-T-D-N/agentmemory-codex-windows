...
const BENCH_PROJECT =
  process.env["AGENTMEMORY_BENCH_PROJECT"]?.trim() ||
  "agentmemory-load-benchmark";
...
async function measureMemoriesLatest(
  ...
  const { latencies, errors, wallMs } = await driveLoad(C, ops, async () => {
    const res = await fetch(`${baseUrl}/agentmemory/memories?project=${encodeURIComponent(BENCH_PROJECT)}&latest=true`, {
      method: "GET",
      signal: AbortSignal.timeout(30_000),
    });
    await res.text().catch(() => "");
